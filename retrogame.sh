...
if [ $RETROGAME_SELECT -lt 9 ]; then
	...
	curl -f -s -o /tmp/retrogame https://raw.githubusercontent.com/MCollier24/Adafruit-Retrogame/master/retrogame
	if [ $? -eq 0 ]; then
	...
	curl -f -s -o /boot/retrogame.cfg https://raw.githubusercontent.com/MCollier24/Adafruit-Retrogame/master/configs/retrogame.cfg.${CONFIGNAME[$RETROGAME_SELECT-1]}
	if [ $? -eq 0 ]; then
		echo "OK"
	else
		echo "ERROR"
